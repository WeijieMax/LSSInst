...
class InstanceBank(nn.Module):
    ...
    def get(self, batch_size, metas=None):
        ...
        if (
            ...
            self.mask = (
                torch.BoolTensor([
                    not single_img_metas['start_of_sequence'] 
                    for single_img_metas in metas['img_metas']]).to(anchor.device)
            )
        else:
            self.cached_feature = None
            self.cached_anchor = None
        ...
        if (
            ...
            time_interval = torch.where(
                torch.logical_or(
                    time_interval == 0, time_interval.abs() > self.max_time_interval
                ),
