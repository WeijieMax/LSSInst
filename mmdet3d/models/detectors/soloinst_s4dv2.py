...
from mmdet3d.core.bbox.util import normalize_bbox_s4dv2
...
class SOLOInst_s4dv2(SOLOFusion):
    def __init__(self,
                 deform_sample=False,
                 embed_dims_debev=256,
                 num_heads=8,
                 num_points=4,
                 num_levels=1,
                 num_bev_queue=1,
                 ...
                 freeze_bev=False,
                 **kwargs):
        super(SOLOInst_s4dv2, self).__init__(**kwargs)
        self.img_neck_s4dv2 = builder.build_neck(img_neck_s4dv2)
        ...
        if self.with_prior_grad and self.with_priors:
        ...
        if self.deform_sample:
            self.embed_dims = embed_dims_debev
    ...
    def image_encoder(self, img):
        ...
        feature_maps = list(self.img_neck_s4dv2(backbone_feats))
        for i, feat in enumerate(feature_maps):
            feature_maps[i] = torch.reshape(
                feat, (B, N) + feat.shape[1:]
            )
        ...
        if self.do_history_stereo_fusion:
            ...
            return feature_maps,neck_feats, stereo_feats.view(B, N, *stereo_feats.shape[1:]), backbone_feats
        else:
            return feature_maps,neck_feats, None, backbone_feats
    ...
    def extract_img_feat(self, img, img_metas, gt_bboxes_3d=None):
        ...
        feature_maps, curr_img_encoder_feats, curr_stereo_feats, backbone_feats = self.image_encoder(imgs[0])
        ...
        return x, feature_maps, depth_digit, backbone_feats
    def prepare_s4dv2(self, img_input, img_metas,kwargs,gt_bboxes_3d,gt_labels_3d):
        img_shape = torch.tile(torch.tensor(img_input.shape[-2:]),[*img_input.shape[:2],1]).to(img_input.device)
    ...
    def forward_train(self,
        ...
        img_feats, feature_maps, depth, backbone_feats = self.extract_img_feat(img_inputs, img_metas)
        data = self.prepare_s4dv2(img_inputs[0], img_metas,kwargs,gt_bboxes_3d,gt_labels_3d)
        ...
        if self.post_pts_bbox_head:
            ...
            priors = None
            if self.with_priors:
                priors = self.prepare_post_priors([img_feats_post], bbox_pts)
            feature_queue = None
            meta_queue = None
            cls_scores, reg_preds = self.post_pts_bbox_head(
                feature_maps, data, feature_queue, meta_queue, priors
            )
            # if self.use_deformable_func:
            feature_maps = DAF.feature_maps_format(feature_maps, inverse=True)
            losses_post_pts = self.post_pts_bbox_head.loss(cls_scores, reg_preds, data, feature_maps)
    ...
    def prepare_post_priors(self, bev_feats, results):
        ...
        for result in results:
            ...
            bbox = normalize_bbox_s4dv2(bbox)
            bbox_padding = torch.Tensor([0])[None].repeat(len(bbox), 1)
            bbox = torch.cat((bbox, bbox_padding), dim=1)
            if len(bbox) < self.num_proposal:
                N = self.num_proposal - len(bbox)
                bbox_padding_xyz = torch.rand(N, 3)
                bbox_padding_other = torch.Tensor([1, 1, 1, 1, 0, 0, 0, 0])[None].repeat(N, 1) # l w h sin cos vx vy
                bbox_padding = torch.cat([bbox_padding_xyz, 
                                        bbox_padding_other], dim=-1)
        ...
        if self.with_prior_grad:
            bev_feat = self.prior_refiner(bev_feat)
    ...
    def simple_test_post_pts(self, bev_feats, img_feats, img, bbox_pts, img_metas, rescale=False):
        ...
        return bbox_results
    def prepare_s4dv2_test(self, img_input, img_metas,kwargs):
        img_shape = torch.tile(torch.tensor(img_input.shape[-2:]),[*img_input.shape[:2],1]).to(img_input.device)
    ...
    def simple_post_test(self,
        ...
        bev_feats, feature_maps, depth, backbone_feats = self.extract_img_feat(img, img_metas)
        # kwargs['projection_mat'] = kwargs['projection_mat'][0]
        data = self.prepare_s4dv2_test(img[0], img_metas,kwargs)
        img_feats_post = bev_feats[0].clone().detach()
        bbox_pts = self.simple_test_pts([img_feats_post], img_metas, rescale=False)
        priors = None
        if self.with_priors:
            priors = self.prepare_post_priors([img_feats_post], bbox_pts)
        ...
        results = self.post_pts_bbox_head.post_process(cls_scores, reg_preds)
